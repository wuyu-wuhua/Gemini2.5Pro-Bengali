...
function getSafeTranslations() {
    ...
    return {
        ...
        bn: { 'untitled-chat': 'Без названия чата', 'history-title': 'История чата', 'no-history': 'Еще нет истории', 'welcome-message': 'Привет! Чем могу помочь?' }
    };
}
...
function switchLanguage(lang) {
    if (lang && lang === 'bn') { // Only bn is supported now
...
function applyTranslations(languageToApply) {
    const currentLang = languageToApply || localStorage.getItem('language') || 'bn';
    const trans = getSafeTranslations();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    langOptions.forEach(option => {
        ...
        // Since only bn is supported, ensure only bn option can be active if it exists
        if (option.getAttribute('data-lang') !== 'bn') option.style.display = 'none'; else option.style.display = 'block';
    });
...
if (langOptions) {
    langOptions.forEach(option => {
        ...
        newOption.addEventListener('click', (e) => {
            ...
            if (lang === 'bn') { // Only allow switching to bn
                switchLanguage(lang);
            }
        });
    });
